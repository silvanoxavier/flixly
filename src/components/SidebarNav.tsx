...
export default function SidebarNav({ expanded, onNavClick }: SidebarNavProps) {
  ...
  return (
    <nav className="flex flex-col h-full p-3 space-y-4">
      {/* Scrollable main navigation */}
      <div className="flex-1 overflow-y-auto space-y-1.5 scrollbar-thin scrollbar-thumb-muted-foreground/50 scrollbar-track-transparent pr-2">
        {navItems.map((item) => (
          <div key={item.label}>
            {item.href ? (
              <NavLink
            ...
            {item.children && (expanded || openSubmenu === item.label) && (
              <div className={`pl-6 ${expanded ? 'block pb-2' : 'absolute left-full top-0 bg-card shadow-lg rounded-md py-2 w-48 z-50'}`}>
